...
/*
using opentelemetry::sdk::context::Context;
using opentelemetry::common::AttributeValue;
using opentelemetry::nostd::get;
using opentelemetry::nostd::string_view;
using opentelemetry::nostd::bad_variant_access;
*/
...
TEST(ContextTest, ContextImmutability)
{
  ...
  EXPECT_THROW(nostd::get<nostd::string_view>(test_context.GetValue(test_key)), nostd::bad_variant_access);
  EXPECT_THROW(nostd::get<nostd::string_view>(test_context.GetValue(foo_key)), nostd::bad_variant_access);
}
...
// Tests that two empty lists return true when compared
TEST(ContextTest, ContextCompareEmptyContexts){
  context::Context test_context = context::Context();
  context::Context foo_context = context::Context();

  EXPECT_TRUE(test_context == foo_context);
}
...
// Tests that after assignment, a copy constructor returns true
TEST(ContextTest, ContextCompareAssignedContext){
  using M                          = std::map<std::string, common::AttributeValue>;
